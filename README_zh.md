...
vco webclick http://127.0.0.1:9005 --fill 输入消息=你好 --target 发送
                                  ...
                                  # --panel http://127.0.0.1:8767 改在 vco debug 面板那个
                                  #   浏览器里点，人在面板上看得见，见下
...
vco debug http://127.0.0.1:3080/
                                  # 你在旁边看着、自己也动手的浏览器；锁定元素→导出「哪里错了、谁错了」
                                  # 面板 :8767；webclick --panel 可以让自动化跑在这个浏览器里
```
...
`webclick`/`webrun` 是"让机器替你操作"，`vco debug` 是反过来的那一半：**一个你在旁边看着、自己也动手的浏览器**。

```bash
vco debug http://127.0.0.1:3080/          # 面板开在 http://127.0.0.1:8767/
```

左栏是受控浏览器的实时画面，右栏是现场记录。手势只有一个约定：

| 动作 | 结果 |
|---|---|
| 普通点击 / 打字 | 真的操作那个页面：按钮真触发、链接真跳转、中文能输入、⌘V 真粘贴、快捷键真生效 |
| ⌘/Ctrl/Alt + 点击 | 锁定这个元素，右侧多一张卡片；再点别的继续加，同一元素不重复 |

**它只做信息层：抓现场、导出报告。** 不接模型、不改页面（原先那套 `/api/llm` + patch schema 已经删掉）。

- **导出**一份 Markdown，回答两件事：**哪里错了**（选择器 / 位置 / 路径）和**谁错了**
  （向上找第一个带 `id`/`role`/`aria-label`/`data-*` 的归属容器——元素自己的 class 常是 CSS-module 哈希没法 grep，
  而 `data-slot="sidebar.brand.name"` 能直接落到组件上）。附控制台错误与失败请求。
  另有「下载标注截图」：当前帧 + 与卡片同编号的橙框。
- **运行记录**：只读的 run 时间线，把 `webclick`/`webrun`/`watch` 留下的 `events.jsonl`
  每一步（含该步截图，报错标红）显示出来。面板是你看页面的地方，也是你看自动化干了什么的地方。
- **Safari 也能用**：WebSocket 握手必须回 `HTTP/1.1 101`——回 HTTP/1.0 会被 Safari 拒掉，
  面板永远空白且**一句报错都没有**。连接断了会显示红色横幅并自动重连；右栏「连接与输入诊断」
  直接给出 socket 状态、输入事件计数和浏览器 UA，点了没反应时先看它。

### 让自动化跑在你看得见的浏览器里

`webclick` 默认自己另起一个无头浏览器，你看不见它。加 `--panel` 改成驱动**面板那个**浏览器：

```bash
vco debug http://127.0.0.1:3080/ &                                    # 面板 :8767
vco webclick http://127.0.0.1:3080/ --target "New Session" \
             --panel http://127.0.0.1:8767 --dir cache/runs/click1
```

目标会被橙色光圈圈住、停 0.9 秒再点下去，你在左栏看着它发生；右侧的 DOM 卡片、hover 读数、
导出报告照样能用，因为它们看的是同一个页面。定位方式、唯一命中才点、多候选拒绝、`--expect` 校验、
结果 JSON、事件留档都和独立模式完全一致——切换只是一个 flag。
...
```bash
python3 -m pytest tests/ -q                          # 单测：ws 帧解码、cookie 算法、面板契约
PANEL_URL=http://127.0.0.1:8767/ python3 tests/e2e_debug_panel.py
```

端到端（40 项）用真浏览器驱动真面板：连接与首帧、锁定累积与去重、标注不丢、普通点击真触发、
链接真跳转、打字/粘贴/中文/组合键、报告里的"哪里/谁"、运行时间线、`webclick --panel`。
**必须再跑一遍 `ENGINE=webkit`**（Safari 内核）——只跑 Chromium 的绿灯不算证据，
`101` 用 HTTP/1.0 那次就是这么漏过去的。
